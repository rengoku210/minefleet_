...
function getCanonicalPublicUrl() {
...
var WINDOWS_INSTALLER_SCRIPT = `# MineFleet Agent Installer for Windows
...
`;
var LINUX_INSTALLER_SCRIPT = `#!/usr/bin/env bash
set -euo pipefail
...
`;
  ...
  const handleInstallPs1 = async (request, reply) => {
...
${targetMarker}`);
  ...
  const handleInstallSh = async (request, reply) => {
    const { token = "", controller = "" } = request.query || {};
    const effectiveController = controller || config.controllerUrl || "https://minefleet.vercel.app";
  ...
  const handleAgentBundle = async (request, reply) => {
    const bundle = getAgentBundleContent();
    return reply.header("Content-Type", "application/javascript; charset=utf-8").header("Cache-Control", "public, max-age=300").header("Content-Disposition", 'attachment; filename="minefleet-agent.js"').send(bundle);
  };
